...
class APTMotor():
    def __init__(self, SerialNum=None, HWTYPE=42, verbose=False):
        ...
        # TODO : Error reporting to know if initialisation went successfully or not.
    ...
    def mcRel(self, relDistance, moveVel=0.5):
        ...
        self.setVel(maxVel)
        if self.verbose: print('mcRel SUCCESS')
        return True
    ...
    def mcAbs(self, absPosition, moveVel=0.5):
        ...
        self.setVel(maxVel)
        if self.verbose: print('mcAbs SUCCESS')
        return True

    def mbRel(self, relDistance):
        '''
        Moves the motor a relative distance specified with backlash correction
        relDistance    float     Relative position desired
        '''
        ...
        if self.verbose: print('mbRel SUCCESS')
        return True

    def mbAbs(self, absPosition):
        '''
        Moves the motor to the Absolute position specified with backlash correction
        absPosition    float     Position desired
        '''
        ...
        self.mAbs(absPosition)
        if self.verbose: print('mbAbs SUCCESS')
        return True
    ...
    def go_home(self):
        ...
        if not self.Connected:
            raise Exception('Please connect first! Use initializeHardwareDevice')
        if self.verbose: print('go_home SUCCESS')
